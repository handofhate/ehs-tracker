...
function migrateState(s) {
  const migrated = V2_STATE.migrateState(s, {
    idFactory: uid,
    today,
    clientColumnKeys: CLIENT_COLS.map(c => c.key),
    normalizeOwedInclude,
    sanitizeTheme: _sanitizeTheme
  });
  const feeMigration = V2_FEES.migrateStateFees(migrated, { today });
    ...
    while (year < 2100) {
      ...
      if (!seenDates.has(billDate)) {
        hw.payments.push({
          ...
          status: 'invoiced',
          autoGenerated: true,
          feeConfig: V2_FEES.createFeeConfig(state.settings, billDate)
        });
...
function hwDetail(hw, c) {
  ...
  const feeLabel  = 'Est. Square fees (locked per payment)';
...
function jobDetail(job, c) {
  ...
  const en     = esc(emp?.name || 'Employee');
  const admin  = currentUser?.isAdmin;
  const jobFee = V2_FEES.forJob(job, state.settings);
  const jobFeeLabel = `${(jobFee.feeRate * 100).toFixed(1)}%${jobFee.txnFee ? ` + $${jobFee.txnFee.toFixed(2)}/txn` : ''}`;
      ...
      <div class="detail-section">
        ...
        <div class="admin-only">
          <div class="line-item mobile-fee-row" style="padding-top:6px">
            <div class="line-item-label" style="font-size:16px"><span class="fee-label-main">Est. Square fees</span><span class="fee-label-detail"> (~${jobFeeLabel})</span></div>
...
function saveSettings() {
  ...
  if (feeChanged) {
    showConfirm(
      `This changes the default fee settings for new jobs and new HomeWatch payments from ${(oldFeeRate * 100).toFixed(2)}% + $${oldTxnFee.toFixed(2)} per transaction to ${(draft.feeRate * 100).toFixed(2)}% + $${draft.txnFee.toFixed(2)}. Existing job and payment records keep their locked fee snapshots. Continue?`,
